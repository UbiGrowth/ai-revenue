...
-- Create policies (workspace_members pattern from existing schema)
CREATE POLICY "Users can manage their workspace connections"
  ON google_workspace_connections
  FOR ALL
  USING (
    tenant_id IN (
...
-- Triggers for auto-marking items for AI analysis
...
CREATE TRIGGER gmail_mark_for_analysis
  ...
  FOR EACH ROW
  EXECUTE FUNCTION mark_for_ai_analysis();
...
BEGIN
  -- This is a placeholder - actual refresh happens in Edge Function
  RETURN QUERY
  SELECT
    gwc.access_token,
    gwc.token_expires_at
  FROM google_workspace_connections gwc
  WHERE gwc.tenant_id = p_tenant_id
    AND gwc.is_active = true;
END;
