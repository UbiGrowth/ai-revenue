/**
 * Social-Deploy Rate Limit Integration Tests
 * 
 * Tests rate limiting for social-deploy endpoint (userId + IP keyed)
 * Limits: 10/min, 60/hr, 500/day
 * 
 * Prerequisites:
 ...
 * 
 * Run with:
 * TEST_RATE_LIMITS=true TEST_AUTH_TOKEN_U1=xxx TEST_AUTH_TOKEN_U2=xxx npx vitest run src/test/social-deploy-rate-limit.test.ts
 */
...
describe.skipIf(!shouldRun || !authTokenU1)("Social-Deploy Rate Limit Tests", () => {
  const limits = RATE_LIMITS["social-deploy"]; // 10/min, 60/hr, 500/day
  const endpoint = "social-deploy";
      ...
      const results = await callEdgeFunctionNTimes(
        {
          ...
          body: { assetId: `test-asset-${generateTestId()}` },
  ...
  describe("Key isolation - Different IP", () => {
    const ip1 = "11.11.11.3";
    const ip2 = "11.11.11.4";
